 ...
 */
class RedBlackTree<Key : Comparable<Key>, Value>(vararg pairs: Pair<Key, Value>) {
    ...
    private fun height(x: Node<Key, Value>?): Int {
        if (x == null) return -1
        return 1 + height(x.left).coerceAtLeast(height(x.right))
    }
    ...
    private fun depth(x: Node<Key, Value>?, k: Key): Int {
        ...
        return when {
            cmp > 0 -> 1 + depth(x.right, k)
            cmp < 0 -> 1 + depth(x.left, k)
            else -> 1
        }
    }
